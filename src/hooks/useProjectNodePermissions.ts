  ...
  return useMutation({
    mutationFn: async ({ nodeId, user_id, permission }: ProjectNodePermissionDto & { nodeId: number }) => {
      const response = await axios.post(
        `${API_URL}/project-nodes/${nodeId}/assign_permission/`,
        { user_id, permission },
        axiosConfig
      );
...
export const useRemoveProjectNodePermission = () => {
  ...
  return useMutation({
    mutationFn: async ({ nodeId, user_id, permission }: ProjectNodePermissionDto & { nodeId: number }) => {
      const response = await axios.post(
        `${API_URL}/project-nodes/${nodeId}/remove_permission/`,
        { user_id, permission },
        axiosConfig
      );
      return response.data;
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await axios.get(
        `${API_URL}/project-nodes/${nodeId}/user_permissions/?user_id=${userId}`,
        axiosConfig
      );
